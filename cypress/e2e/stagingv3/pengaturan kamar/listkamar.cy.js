...
// Bug-target: TC-011 (search by Instansi -> pola BUG-001), TC-009 (filter kombinasi -> pola BUG-009/014).
...
describe('Kamar — List (CARDS School)', () => {
  ...
  it('TC-KMR-LST-011 | Positif | Cari by Instansi [KANDIDAT BUG-001]', () => {
    const nama = uniq();
    seed(d.instansi.primary, nama);
    kamar.search(d.instansi.primary); // "SDIT"
    // PRD: cari by Instansi harus nampilin kamar instansi tsb (seed kita, newest -> teratas).
    //   Kalau empty padahal data ada = BUG (pola BUG-001 Mapel: search by Instansi returns empty).
    kamar.assertRowExists(nama);
  });

  it('TC-KMR-LST-012 | Positif | Cari by PIC', () => {
    const nama = uniq();
    kamar.openAddModal();
